...
**Pinned by:** the test asserting current behaviour, by path and name.
**Expires when:** what would have to change for this entry to be deleted. "Never" is a valid answer
for a deliberate deviation; for an accepted-wrong entry it usually means the entry is really debt,
and belongs in `PLAN.md` as well.
