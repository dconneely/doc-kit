...
`.claude/skills/doc-kit-adopt/SKILL.md`, the conventional Claude Code project-skill location. Claude
Code's own documentation map also lists `.agents/skills` as a recognised project-level skill
...
signals - the same way root `AGENTS.md` does - that this is written for coding agents generally, not
exclusively for one vendor's tool. Keeping both was rejected: a `SKILL.md` is not the kind of
single-purpose pointer file `CLAUDE.md`->`AGENTS.md` aliasing suits (see `DOC-MAP.md` §2.7); it is
the skill's entire content, and there is nothing for a second copy to point at without duplicating
it.
