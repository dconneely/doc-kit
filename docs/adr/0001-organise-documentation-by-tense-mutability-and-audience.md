...
Documentation rots in a characteristic way. It is rarely that nobody writes anything; it is that
one file accumulates several jobs - part record of work done, part backlog, part architecture
...
has to be filed somewhere *within* it - so the problem reappears one level down. Every project that
